...
PUBLISHED=$(npm view oh-my-patoniagent version 2>/dev/null || echo "not published")
...
<project>oh-my-patoniagent (npm package)</project>
...
GOAL: Review all changes heading into npm publish of oh-my-patoniagent. These changes span {COMMIT_COUNT} commits across {FILE_COUNT} files.
...
BACKGROUND: Pre-publish review of oh-my-patoniagent, an OpenCode plugin with 1268 TypeScript files, 160k LOC. Changes since v{PUBLISHED} are about to be published.
...
```markdown
# Pre-Publish Review — oh-my-patoniagent
