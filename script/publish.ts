...
const PACKAGE_NAME = "oh-my-patoniagent"
...
console.log("=== Publishing oh-my-patoniagent (multi-package) ===\n")
...
async function updateAllPackageVersions(newVersion: string): Promise<void> {
  ...
  for (const platform of PLATFORM_PACKAGES) {
    const pkgName = `oh-my-patoniagent-${platform}`
...
async function getContributors(previous: string): Promise<string[]> {
  ...
  try {
    const compare =
      await $`gh api "/repos/gataungapainn/oh-my-patoniagent/compare/v${previous}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
async function gitTagAndRelease(newVersion: string, notes: string[]): Promise<void> {
  ...
  await $`git add package.json assets/oh-my-patoniagent.schema.json`
  for (const platform of PLATFORM_PACKAGES) {
