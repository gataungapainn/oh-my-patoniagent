...
const originalStartupTimeout = process.env.OMPA_OPENCLAW_REPLY_LISTENER_STARTUP_TIMEOUT_MS
...
afterEach(() => {
  resetStateDir()
  process.env.OMPA_OPENCLAW_REPLY_LISTENER_STARTUP_TIMEOUT_MS = "25"
})
...
afterAll(() => {
  ...
  if (originalStartupTimeout === undefined) {
    delete process.env.OMPA_OPENCLAW_REPLY_LISTENER_STARTUP_TIMEOUT_MS
  } else {
    process.env.OMPA_OPENCLAW_REPLY_LISTENER_STARTUP_TIMEOUT_MS = originalStartupTimeout
  }
