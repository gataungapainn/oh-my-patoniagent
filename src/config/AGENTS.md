...
32 schema files composing `OhMyPatoniAgentConfigSchema`. Zod v4 validation with `safeParse()`. All fields optional — omitted fields use plugin defaults.

## SCHEMA TREE

```
config/schema/
├── oh-my-patoniagent-config.ts    # ROOT: OhMyPatoniAgentConfigSchema (composes all below)
...
2. Add field to `oh-my-patoniagent-config.ts` root schema
