...
function shouldDisablePostHog(): boolean {
  if (process.env.OMPA_DISABLE_POSTHOG === "true" || process.env.OMPA_DISABLE_POSTHOG === "1") {
    return true
  }

  return isFalsy(process.env.OMPA_SEND_ANONYMOUS_TELEMETRY?.trim().toLowerCase())
}
