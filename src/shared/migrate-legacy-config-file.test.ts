...
describe("migrateLegacyConfigFile", () => {
  ...
  describe("#given oh-my-patoniagent.jsonc exists but oh-my-patoniagent.jsonc does not", () => {
    describe("#when migrating the config file", () => {
      it("#then writes oh-my-patoniagent.jsonc and renames the legacy file to a backup", () => {
        const legacyPath = join(testDir, "oh-my-patoniagent.jsonc")
        const backupPath = join(testDir, "oh-my-patoniagent.jsonc.bak")
        writeFileSync(legacyPath, '{ "agents": {} }')

        const result = migrateLegacyConfigFile(legacyPath)

        expect(result).toBe(true)
        expect(existsSync(join(testDir, "oh-my-patoniagent.jsonc"))).toBe(true)
        expect(existsSync(legacyPath)).toBe(false)
        expect(existsSync(backupPath)).toBe(true)
        expect(readFileSync(join(testDir, "oh-my-patoniagent.jsonc"), "utf-8")).toBe('{ "agents": {} }')
  ...
  describe("#given a legacy config sidecar exists", () => {
    describe("#when migrating the config file", () => {
      it("#then copies applied migration history to the canonical sidecar", () => {
        const legacyPath = join(testDir, "oh-my-patoniagent.json")
        const legacySidecarPath = `${legacyPath}.migrations.json`
        const canonicalSidecarPath = join(testDir, "oh-my-patoniagent.json.migrations.json")
  ...
  describe("#given oh-my-patoniagent.json exists but oh-my-patoniagent.json does not", () => {
    describe("#when migrating the config file", () => {
      it("#then copies to oh-my-patoniagent.json", () => {
        const legacyPath = join(testDir, "oh-my-patoniagent.json")
        writeFileSync(legacyPath, '{ "agents": {} }')
        ...
        expect(result).toBe(true)
        expect(existsSync(join(testDir, "oh-my-patoniagent.json"))).toBe(true)
      })
    })
  })

  describe("#given oh-my-patoniagent.jsonc already exists", () => {
    describe("#when attempting migration", () => {
      it("#then returns false and does not overwrite", () => {
        const legacyPath = join(testDir, "oh-my-patoniagent.jsonc")
        const canonicalPath = join(testDir, "oh-my-patoniagent.jsonc")
      ...
      it("#then does not copy legacy team_mode.tmux_visualization into the canonical file", () => {
        const legacyPath = join(testDir, "oh-my-patoniagent.json")
        const canonicalPath = join(testDir, "oh-my-patoniagent.json")
  ...
  describe("#given the file does not exist", () => {
    describe("#when attempting migration", () => {
      it("#then returns false", () => {
        const result = migrateLegacyConfigFile(join(testDir, "oh-my-patoniagent.jsonc"))
