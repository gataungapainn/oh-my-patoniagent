...
export function logLegacyPluginStartupWarning(deps: LogLegacyPluginStartupWarningDeps = {}): void {
  ...
  console.warn(
    `[oh-my-patoniagent] WARNING: Your opencode.json uses the legacy package name "${LEGACY_PLUGIN_NAME}".`
  ...
  if (migrated) {
    console.warn(`[oh-my-patoniagent] Auto-migrated opencode.json: ${result.legacyEntries.join(", ")} -> ${suggestedEntries.join(", ")}`)
  } else {
    console.warn(
      `[oh-my-patoniagent] Could not auto-migrate. Please manually update your opencode.json:`
