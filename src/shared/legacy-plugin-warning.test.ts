...
describe("checkForLegacyPluginEntry", () => {
  it("detects a bare legacy plugin entry", () => {
    ...
    try {
      // given
      writeFileSync(join(testConfigDir, "opencode.json"), JSON.stringify({ plugin: ["oh-my-patoniagent"] }, null, 2))
      ...
      expect(result.legacyEntries).toEqual(["oh-my-patoniagent"])
  ...
  it("detects a version-pinned legacy plugin entry", () => {
    ...
    try {
      // given
      writeFileSync(join(testConfigDir, "opencode.json"), JSON.stringify({ plugin: ["oh-my-patoniagent@3.10.0"] }, null, 2))
      ...
      expect(result.hasLegacyEntry).toBe(true)
      expect(result.hasCanonicalEntry).toBe(false)
      expect(result.legacyEntries).toEqual(["oh-my-patoniagent@3.10.0"])
    } finally {
      cleanupTestConfigDir(testConfigDir)
    }
  })
