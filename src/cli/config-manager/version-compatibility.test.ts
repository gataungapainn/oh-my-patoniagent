...
describe("extractVersionFromPluginEntry", () => {
  it("extracts version from canonical plugin entry", () => {
    const version = extractVersionFromPluginEntry("oh-my-patoniagent@3.15.0")
    expect(version).toBe("3.15.0")
  })

  it("extracts version from legacy plugin entry", () => {
    const version = extractVersionFromPluginEntry("oh-my-patoniagent@3.14.0")
    expect(version).toBe("3.14.0")
  })

  it("returns null for bare plugin entry", () => {
    const version = extractVersionFromPluginEntry("oh-my-patoniagent")
    expect(version).toBeNull()
  })

  it("handles prerelease versions", () => {
    const version = extractVersionFromPluginEntry("oh-my-patoniagent@3.16.0-beta.1")
