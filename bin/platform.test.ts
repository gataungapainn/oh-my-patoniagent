...
describe("getBinaryPath", () => {
  test("returns path without .exe for Unix platforms", () => {
    ...
    expect(result).toBe("oh-my-opencode-darwin-arm64/bin/oh-my-patoniagent");
  });
  ...
  test("returns path with .exe for Windows", () => {
    ...
    expect(result).toBe("oh-my-opencode-windows-x64/bin/oh-my-patoniagent.exe");
  });
  ...
  test("returns path without .exe for Linux", () => {
    ...
    // #then returns path without extension
    expect(result).toBe("oh-my-opencode-linux-x64/bin/oh-my-patoniagent");
  });
});
