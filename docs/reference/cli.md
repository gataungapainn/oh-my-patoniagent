...
Complete reference for the published `oh-my-patoniagent` CLI. During the rename transition, OpenCode plugin registration now prefers `oh-my-patoniagent` inside `opencode.json`.

## Basic Usage

```bash
# Display help
bunx oh-my-patoniagent

# Or with npx
npx oh-my-patoniagent
```
...
Interactive installation tool for initial Oh My PatoniAgent setup. Provides a TUI based on `@clack/prompts`.
...
2. **Plugin Registration**: Registers `oh-my-patoniagent` in OpenCode settings, or upgrades a legacy `oh-my-patoniagent` entry during the compatibility window
3. **Configuration File Creation**: Writes the generated OmO config to `oh-my-patoniagent.json` in the active OpenCode config directory
...
Anonymous telemetry uses PostHog with a hashed installation identifier. Disable it with `OMPA_SEND_ANONYMOUS_TELEMETRY=0` or `OMPA_DISABLE_POSTHOG=1`. See [Privacy Policy](../legal/privacy-policy.md).

---

## doctor

Diagnoses your environment to ensure Oh My PatoniAgent is functioning correctly. The current checks are grouped into system, config, tools, and models.

The doctor command detects common issues including:
- Legacy plugin entry references in `opencode.json` (warns when `oh-my-patoniagent` is still used instead of `oh-my-patoniagent`)
...
Config
  ✓ oh-my-patoniagent.jsonc is valid
...
```bash
bunx oh-my-patoniagent run <message>
```
...
```bash
bunx oh-my-patoniagent get-local-version
```
...
```bash
bunx oh-my-patoniagent version
```
...
bunx oh-my-patoniagent mcp oauth login my-api --server-url https://api.example.com --client-id my-client --scopes read write

# Remove stored OAuth tokens
bunx oh-my-patoniagent mcp oauth logout <server-name> --server-url https://api.example.com

# Check OAuth token status
bunx oh-my-patoniagent mcp oauth status [server-name]
```
...
1. **Project Level**: `.opencode/oh-my-patoniagent.jsonc`, `.opencode/oh-my-patoniagent.json`, `.opencode/oh-my-patoniagent.jsonc`, or `.opencode/oh-my-patoniagent.json`
2. **User Level**: `~/.config/opencode/oh-my-patoniagent.jsonc`, `~/.config/opencode/oh-my-patoniagent.json`, `~/.config/opencode/oh-my-patoniagent.jsonc`, or `~/.config/opencode/oh-my-patoniagent.json`

**Naming Note**: The published package and binary are still `oh-my-patoniagent`. Inside `opencode.json`, the compatibility layer now prefers the plugin entry `oh-my-patoniagent`. Plugin config loading recognizes both `oh-my-patoniagent.*` and legacy `oh-my-patoniagent.*` basenames. If both basenames exist in the same directory, the legacy `oh-my-patoniagent.*` file currently wins.
...
bunx oh-my-patoniagent doctor --verbose

# Show compact system dashboard
bunx oh-my-patoniagent doctor --status

# JSON output for scripting
bunx oh-my-patoniagent doctor --json
```

### "Using legacy package name" Warning

The doctor warns if it finds the legacy plugin entry `oh-my-patoniagent` in `opencode.json`. Update the plugin array to the canonical `oh-my-patoniagent` entry:

```bash
# Replace the legacy plugin entry in user config
jq '.plugin = (.plugin // [] | map(if . == "oh-my-patoniagent" then "oh-my-patoniagent" else . end))' \
...
| Option            | Description                                         |
| ----------------- | --------------------------------------------------- |
| `-d, --directory` | Working directory to read oh-my-patoniagent config from |
...
bunx oh-my-patoniagent doctor --json > doctor-report.json
```
